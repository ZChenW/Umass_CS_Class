...
def prepare(sp, n=100, test_ration=0.2, threshold=False):
    train_acc = []
    test_acc = []
...
def make_graph_plt(acc, title, dir, filename=None, name=None):
    ...
    plt.xlabel("Accuracy")
    plt.ylabel(f"Frequency on {name} Data")
    plt.title(title)
    plt.grid()
    #    plt.ylim(0, 1)
    # plt.xlim(0.90, 1.05)
...
def main():
    ...
    train_acc, test_acc = prepare(read_csv_file(csv_path), n=100, threshold=True)
