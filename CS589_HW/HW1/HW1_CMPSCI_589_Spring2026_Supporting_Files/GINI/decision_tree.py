...
class DecisionTree:
    ...
    def getTree(self, data, threshod=False):
        ...
        for a, dateframe in self.SpliteByFeature(data, bestfeature):
            tree[bestfeature][a] = self.getTree(dateframe, threshod)

        return tree

    def fit(self, train, threshod=False):
        self.tree = self.getTree(train, threshod)
