...
def x(k=5, max_length=10, min_samples_split=2):
    ...
    raisin_csv_path = "../dataset/raisin.csv"
    raisin_csv_label_name = "label"

    titanic_csv_path = "../dataset/titanic.csv"
    titanic_csv_label_name = "label"
    ...
    raisin_result = cal_one_dataset(
        dataset_name=f"Raisin_{k}_depth_{max_length}_minsplit{min_samples_split}",
        csv_path=raisin_csv_path,
        label_col=raisin_csv_label_name,
        dir=f"../picture/raisin_k{k}_depth{max_length}_minsplit{min_samples_split}",
        ntree_values=(1, 5, 10, 20, 30, 40, 50),
        k=k,
        random_seed=42,
        ...
        min_samples_split=min_samples_split,
        min_gain=1e-6,
        node_chose_limit=2,  # [\sqrt(7)] = 2
        positive_label=1,
    )

    titanic_result = cal_one_dataset(
        dataset_name=f"titanic_{k}_depth_{max_length}_minsplit{min_samples_split}",
        csv_path=titanic_csv_path,
        label_col=titanic_csv_label_name,
        dir=f"../picture/titanic_k{k}_depth{max_length}_minsplit{min_samples_split}",
        ntree_values=(1, 5, 10, 20, 30, 40, 50),
        k=k,
        random_seed=42,
        max_lenght=max_length,
        min_samples_split=min_samples_split,
        min_gain=1e-6,
        node_chose_limit=2,  # [\sqrt(6)] = 2
        positive_label=1,
    )

    print(
        f"k = {k}, max_depth = {max_length}, min_samples_split = {min_samples_split} Raisin Result:"
    )
    print(raisin_result)
    print(
        f"\nk = {k}, max_depth = {max_length}, min_samples_split = {min_samples_split} Titanic Result:"
    )
