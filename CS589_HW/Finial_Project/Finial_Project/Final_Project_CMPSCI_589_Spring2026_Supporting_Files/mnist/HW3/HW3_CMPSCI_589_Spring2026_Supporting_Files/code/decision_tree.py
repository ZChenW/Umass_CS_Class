...
class DecisionTree:
    def __init__(self, max_lenght=None, min_samples_split=2, min_gain=1e-6, node_chose_limit=None, random_seed=None):
        ...
        # lambda, bu xie la
        self._number = lambda data: pd.api.types.is_numeric_dtype(data)

    
    ## Cal entropy
    def InformationEntropy(self, data):
        return (lambda y: (lambda p: np.sum(-p * np.log2(p)))(y.value_counts().values / len(y)))(data.iloc[:, -1])
    ...
    def NumericInformationGain(self, data, a):
        return (lambda Ent, threshold, left, right: (0.0, threshold)if len(left) == 0 or len(right) == 0 else(Ent - ((len(left) / len(data))*self.InformationEntropy(left) + len(right) / len(data)*self.InformationEntropy(right)), threshold))(self.InformationEntropy(data),data[a].mean(),data[data[a] <= data[a].mean()],data[data[a] > data[a].mean()])
