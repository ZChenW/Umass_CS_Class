    ...
    for arch in arch_linux:
        for mint in lambdas:
            ...
            for i in range(k):
                ...
                y_tain_1 = one_hot(y_tain, num=10)
                y_test_1 = one_hot(y_test, num=10)
...
def prepossing_loan(df, label_name="label"):
    df = df.copy()
...
if __name__ == "__main__":
    ...
    wdbc_input_dim = wdbc_df.shape[1] - 1
    """
    ...
    wdbc_result = train_test_on_fold(
        ...
        arch_linux=[
            ...
            [wdbc_input_dim, 8, 1],
            [wdbc_input_dim, 16, 1],
            [wdbc_input_dim, 8, 4, 1],
            [wdbc_input_dim, 16, 8, 4, 1], 
            [wdbc_input_dim, 16, 8, 4, 2, 1],
        ],
    ...
    ######## MNIST ##########
    mnist_csv_path = "../datasets/digits_dataset.csv"
    mnist_csv_label_name = "label"

    mnist_df = read_csv(mnist_csv_path)
    dir = "../result/"
    os.makedirs(dir, exist_ok=True)
    mnist_input_dim = mnist_df.shape[1] - 1

    mnist_result = train_test_on_fold(
        mnist_df,
        label_name=mnist_csv_label_name,
        arch_linux=[
            [mnist_input_dim, 12, 10],
            [mnist_input_dim, 16, 10],
            [mnist_input_dim, 32, 10],
            [mnist_input_dim, 64, 10],
            [mnist_input_dim, 32, 16, 10],
            [mnist_input_dim, 64, 32, 16, 10],
        ],
        lambdas=[0.01, 0.05, 0.1, 0.3],
        k=10,
        random_seed=42,
    ...
    mnist_result.to_csv(os.path.join(dir, "mnist_result.csv"), index=False)

    mnist_result["index"] = range(1, len(mnist_result) + 1)
    ...
    make_graph_plt(
        ...
        title="NN - Macro-F1",
        out_dir="../picture/",
        filename="digits_nn_f1.png",
    )

    print(mnist_result)

    """
    mnist_best_result = learning_curve(
        mnist_df,
        label_name=mnist_csv_label_name,
        best_arch=[wdbc_input_dim, 2, 1],
        best_lambda=0.01,
