...
def prepossing(df, label_name="label"):
    ...
    df = df[[c for c in df.columns if c != label_name] + [label_name]]
    return df
...
def cal_one_dataset(
    ...
    df = prepossing(df, label_name="label")
...
def x(k=10, max_length=10, min_samples_split=2):
    ...
    credit_csv_path = "../dataset/credit_approval.csv"
    credit_csv_label_name = "label"

    df = read_csv(credit_csv_path, label_name="label")
    df = prepossing(df, label_name="label")
    ...
    credit_result = cal_one_dataset(
        dataset_name=f"credit_{k}_depth_{max_length}_minsplit{min_samples_split}",
        csv_path=credit_csv_path,
        label_col=credit_csv_label_name,
        dir=f"../picture/credit_k{k}_depth{max_length}_minsplit{min_samples_split}",
        ntree_values=(1, 5, 10, 20, 30, 40, 50),
        k=k,
        random_seed=42,
        ...
        min_samples_split=min_samples_split,
        min_gain=1e-6,
        node_chose_limit=math.isqrt(df.shape[1] - 1),  # [\sqrt(15)] = 3
        positive_label=1,
    )

    print(
        f"k = {k}, max_depth = {max_length}, min_samples_split = {min_samples_split} Credit Result:"
    )
    print(credit_result)
