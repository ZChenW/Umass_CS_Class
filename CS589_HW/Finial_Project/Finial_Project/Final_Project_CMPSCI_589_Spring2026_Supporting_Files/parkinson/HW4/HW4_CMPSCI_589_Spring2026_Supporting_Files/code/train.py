...
if __name__ == "__main__":
    ...
    parkinsons_input_dim = wdbc_df.shape[1] - 1
    """
    ...
    wdbc_result = train_test_on_fold(
        ...
        arch_linux=[
            [parkinsons_input_dim, 2, 1],
            [parkinsons_input_dim, 4, 1],
            [parkinsons_input_dim, 8, 1],
            [parkinsons_input_dim, 16, 1],
            [parkinsons_input_dim, 8, 4, 1],
            [parkinsons_input_dim, 16, 8, 4, 1], 
            [parkinsons_input_dim, 16, 8, 4, 2, 1],
        ],
    ...
    wdbc_best_result = learning_curve(
        wdbc_df,
        label_name=wdbc_csv_label_name,
        best_arch=[parkinsons_input_dim, 2, 1],
        best_lambda=0.01,
        k=5,
        random_seed=42,
        positive_label=1,
    ...
    parkinsons_csv_path = "../datasets/parkinsons_label.csv"
    parkinsons_csv_label_name = "label"

    # loan_csv_path = "../datasets/loan.csv"
    # loan_csv_label_name = "label"

    #################_WDBC_#################
    parkinsons_df = read_csv(parkinsons_csv_path)
    parkinsons_df = prepossing(parkinsons_df)
    dir = "../result/"
    os.makedirs(dir, exist_ok=True)
    parkinsons_input_dim = parkinsons_df.shape[1] - 1

    parkinsons_result = train_test_on_fold(
        parkinsons_df,
        label_name=parkinsons_csv_label_name,
        arch_linux=[
            [parkinsons_input_dim, 2, 1],
            [parkinsons_input_dim, 4, 1],
            [parkinsons_input_dim, 8, 1],
    ...
    parkinsons_result.to_csv(os.path.join(dir, "parkinsons_result.csv"), index=False)
    parkinsons_result["index"] = range(1, len(parkinsons_result) + 1)
    ...
    make_graph_plt(
        ...
        title="NN - Acc",
        out_dir="../picture/",
        filename="parkinsons_nn_acc.png",
    )
    ...
    make_graph_plt(
        ...
        title="NN - F1 ",
        out_dir="../picture/",
        filename="parkinsons_nn_f1.png",
    )

    print(parkinsons_result)
