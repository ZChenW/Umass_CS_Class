...
def cal_one_dataset(
    ...
    df = read_csv(csv_path, label_name="Diagnosis")
...
def x(k=10, max_length=10, min_samples_split=2):
    ...
    parkinsons_csv_path = "../dataset/parkinsons.csv"
    parkinsons_csv_label_name = "Diagnosis"
    ...
    parkinsons_result = cal_one_dataset(
        dataset_name=f"parkinsons_{k}_depth_{max_length}_minsplit{min_samples_split}",
        csv_path=parkinsons_csv_path,
        label_col=parkinsons_csv_label_name,
        dir=f"../picture/parkinsons_k{k}_depth{max_length}_minsplit{min_samples_split}",
        ntree_values=(1, 5, 10, 20, 30, 40, 50),
        k=k,
        random_seed=42,
        ...
        min_samples_split=min_samples_split,
        min_gain=1e-6,
        node_chose_limit=4,  # [\sqrt(22)] = 4
        positive_label=1,
    )

    print(
        f"k = {k}, max_depth = {max_length}, min_samples_split = {min_samples_split} Parkinsons Result:"
    )
