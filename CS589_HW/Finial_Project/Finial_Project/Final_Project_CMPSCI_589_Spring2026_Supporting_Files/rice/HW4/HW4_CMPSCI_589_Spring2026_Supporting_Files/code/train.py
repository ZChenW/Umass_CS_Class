...
if __name__ == "__main__":
    ...
    rice_input_dim = wdbc_df.shape[1] - 1
    """
    ...
    wdbc_result = train_test_on_fold(
        ...
        arch_linux=[
            ...
            [rice_input_dim, 8, 1],
            [rice_input_dim, 16, 1],
            [rice_input_dim, 8, 4, 1],
            [rice_input_dim, 16, 8, 4, 1], 
            [rice_input_dim, 16, 8, 4, 2, 1],
        ],
    ...
    #################_WDBC_#################
    rice_df = read_csv(rice_csv_path)
    rice_df = prepossing(rice_df)
    rice_df["label"] = rice_df["label"].replace(
        {
            "Cammeo": 1,
            "Osmancik": 0,
        }
    ...
    dir = "../result/"
    os.makedirs(dir, exist_ok=True)
    rice_input_dim = rice_df.shape[1] - 1

    rice_result = train_test_on_fold(
        rice_df,
    ...
    rice_result.to_csv(os.path.join(dir, "rice_result.csv"), index=False)
    rice_result["index"] = range(1, len(rice_result) + 1)
